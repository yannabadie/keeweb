...
type TypedIpcRenderer = {
    on<EventName extends keyof DesktopIpcRendererEvents>(
        ...
        listener: (
            ...
            ...args: Parameters<DesktopIpcRendererEvents[EventName]>
        ) => void
    ): void;

    send<EventName extends keyof DesktopIpcMainEvents>(
        event: EventName,
...
export class LauncherElectron {
    ...
    get ipcRenderer(): TypedIpcRenderer {
        return electron.ipcRenderer as TypedIpcRenderer;
    }

    openLink(href: string): void {
        if (/^(http|https|ftp|sftp|mailto):/i.test(href)) {
            electron.shell.openExternal(href).catch(noop);
        }
    }
